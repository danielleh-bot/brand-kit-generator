...
function pushVar(lines, name, value, comment) {
  if (value === undefined || value === null || value === '') return;
  lines.push(`  ${name}: ${value};${comment ? ` /* ${comment} */` : ''}`);
}

// Build a "Name — usage" comment for a colour entry when the name/usage came
// from a real source (extracted-via-CSS-var, refined, or enriched) — these are
// the workshop-readable labels. Skip generic crawler placeholders.
function colorComment(entry) {
  if (!entry) return '';
  const meaningful = entry.named_via === 'css_custom_property'
    || entry.source === 'refined' || entry.source === 'enriched';
  const parts = [];
  if (meaningful && entry.name) parts.push(entry.name);
  const usage = entry.usage_description || (Array.isArray(entry.usage) ? entry.usage.slice(0, 3).join(', ') : entry.usage);
  if (meaningful && usage) parts.push(usage);
  return parts.join(' — ');
}
...
function colorVars(lines, colors) {
  if (!colors) return;
  const c = colors;
  const cc = colorComment;
  if (c.primary?.hex)   pushVar(lines, '--brand-color-primary',   c.primary.hex, cc(c.primary));
  if (c.secondary?.hex) pushVar(lines, '--brand-color-secondary', c.secondary.hex, cc(c.secondary));

  if (c.text) {
    pushVar(lines, '--brand-color-text-primary',   c.text.primary?.hex, cc(c.text.primary));
    pushVar(lines, '--brand-color-text-secondary', c.text.secondary?.hex, cc(c.text.secondary));
    pushVar(lines, '--brand-color-text-tertiary',  c.text.tertiary?.hex, cc(c.text.tertiary));
  }
  if (c.backgrounds) {
    pushVar(lines, '--brand-color-bg-base',      c.backgrounds.base?.hex, cc(c.backgrounds.base));
    pushVar(lines, '--brand-color-bg-section',   c.backgrounds.section?.hex, cc(c.backgrounds.section));
    pushVar(lines, '--brand-color-bg-secondary', c.backgrounds.secondary?.hex, cc(c.backgrounds.secondary));
    pushVar(lines, '--brand-color-bg-dark',      c.backgrounds.dark?.hex, cc(c.backgrounds.dark));
  }
  if (c.accents) {
    pushVar(lines, '--brand-color-accent-warning',  c.accents.warning_yellow?.hex, cc(c.accents.warning_yellow));
    pushVar(lines, '--brand-color-accent-negative', c.accents.negative_red?.hex, cc(c.accents.negative_red));
    pushVar(lines, '--brand-color-accent-positive', c.accents.positive_green?.hex, cc(c.accents.positive_green));
    pushVar(lines, '--brand-color-accent-info',     c.accents.info_blue?.hex, cc(c.accents.info_blue));
  }
}
