...
function colorVars(lines, colors) {
  ...
  if (c.accents) {
    ...
    pushVar(lines, '--brand-color-accent-soft-mint',c.accents.soft_mint?.hex);
    pushVar(lines, '--brand-color-accent-sage',     c.accents.sage_green?.hex);
  }
  if (c.borders?.divider?.hex) {
    pushVar(lines, '--brand-color-divider', c.borders.divider.hex);
  }
  if (c.gradients) {
    pushVar(lines, '--brand-gradient-primary',   c.gradients.primary?.value);
    pushVar(lines, '--brand-gradient-secondary', c.gradients.secondary?.value);
  }
}

function radiusVars(lines, brandKit) {
  const r = brandKit?.border_radius;
  if (!r) return;
  pushVar(lines, '--brand-radius-buttons',  r.buttons);
  pushVar(lines, '--brand-radius-cards',    r.cards);
  pushVar(lines, '--brand-radius-images',   r.images);
  pushVar(lines, '--brand-radius-overlays', r.overlays);
}

function shadowVars(lines, brandKit) {
  const s = brandKit?.shadows;
  if (!s) return;
  pushVar(lines, '--brand-shadow-natural', s.natural);
  pushVar(lines, '--brand-shadow-deep',    s.deep);
  pushVar(lines, '--brand-shadow-sharp',   s.sharp);
}

function buttonVars(lines, brandKit) {
  const b = brandKit?.buttons;
  if (!b) return;
  if (b.primary) {
    pushVar(lines, '--brand-btn-primary-bg',     b.primary.background_color);
    pushVar(lines, '--brand-btn-primary-text',   b.primary.text_color);
    pushVar(lines, '--brand-btn-primary-radius', b.primary.border_radius);
  }
  if (b.outline) {
    pushVar(lines, '--brand-btn-outline-border', b.outline.border);
    pushVar(lines, '--brand-btn-outline-text',   b.outline.text_color);
    pushVar(lines, '--brand-btn-outline-radius', b.outline.border_radius);
  }
  if (b.soft_cta) {
    pushVar(lines, '--brand-btn-soft-bg',     b.soft_cta.background_color);
    pushVar(lines, '--brand-btn-soft-text',   b.soft_cta.text_color);
    pushVar(lines, '--brand-btn-soft-radius', b.soft_cta.border_radius);
  }
}
...
function brandKitToCss(brandKit) {
  ...
  radiusVars(varLines, brandKit);
  shadowVars(varLines, brandKit);
  buttonVars(varLines, brandKit);
