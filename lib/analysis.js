 ...
 */
function computeAnalysis(brandKit, defaults, extra) {
  const opts = extra || {};
  const propertyTable = [];
  let driftCount = 0;
  let missingCount = 0;
  ...
  function tokenProvenance(sourcePath) {
    ...
    if (typeof node === 'object') {
      ...
      if (childSources.length > 0) {
        // enriched/derived/refined are real findings, not fallbacks.
        const REAL = ['extracted', 'derived', 'enriched', 'refined'];
        return childSources.some(s => REAL.includes(s)) ? 'extracted' : 'fallback';
      }
    }
    return 'unknown';
  }

  // enriched/refined values are AI-authored real findings — surface them as a
  // distinct provenance the report can badge, never as a fabricated drift.
  const DISCOVERED = new Set(['enriched', 'refined']);
  ...
  for (const def of PROPERTY_DEFS) {
    ...
    } else if (DISCOVERED.has(provenance)) {
      // AI-discovered value that differs from the default: a real finding.
      status = 'discovered';
      driftCount++;
    } else {
      status = 'drift';
  ...
  const eqMeta = brandKit.metadata && brandKit.metadata.extraction_quality;
  // 5-bucket provenance tally surfaced from the crawler's extraction_quality.
  const qualityBuckets = eqMeta ? {
    extracted: eqMeta.extracted_token_count || 0,
    derived: eqMeta.derived_token_count || 0,
    enriched: eqMeta.enriched_token_count || 0,
    refined: eqMeta.refined_token_count || 0,
    fallback: eqMeta.fallback_token_count || 0,
  } : null;
  ...
  const stats = {
    ...
    extractionQuality: eqMeta,
    qualityBuckets,
    enrichment: brandKit.metadata && brandKit.metadata.enrichment,
  };

  // Layer-3 mapping confidence, if the loader builder ran.
  const mappingQuality = opts.mappingQuality || (opts.mappingSummary ? {
    applied: opts.mappingSummary.applied_count,
    gaps: opts.mappingSummary.gap_count,
    safeIgnored: opts.mappingSummary.safe_ignored_count,
    contrastFallbacks: opts.mappingSummary.contrast_fallback_count,
    fontFallbacks: opts.mappingSummary.font_fallback_count,
  } : null);
  ...
  return { propertyTable, stats, gaps, workflow, advantages, mappingQuality };
}
