...
async function extractBrandKit(page, url) {
  ...
  const extracted = await page.evaluate(() => {
    ...
    // ─── PHASE 1: DEEPER EXTRACTION ─────────────────────────────────
    // Additive token extraction — no existing keys removed or renamed.
    // Closes the deterministic gaps surfaced by comparing the crawler's
    // output to the t-online / CBS / NBC / Leckerschmecker reference
    // kits (richer type hierarchy, shadow registry, radius map,
    // spacing scale, CSS custom properties, show-brand colors, icon
    // catalog, header layer breakdown, button variants, deeper text
    // colors, rgba overlays, divider colors).

    // ---- CSS CUSTOM PROPERTIES (--brand-*, --color-*, etc.) ----
    // Modern publishers expose their literal brand vocabulary as CSS
    // custom properties on :root. Scraping these gives us free color
    // names and a token map that downstream layers can match hex
    // values back to. Computed style only returns custom props that
    // were actually declared on documentElement (or inherited).
    const css_custom_properties = (() => {
      ...
      try {
        ...
        for (let i = 0; i < rootStyle.length; i++) {
          ...
          if (!value) continue;
          // Guess the role from the name prefix (color/spacing/font/radius/shadow).
          const lower = name.toLowerCase();
          let role_guess = null;
          if (/^--(.*-)?(color|colour|bg|background|fg|text|brand|accent|primary|secondary|tertiary)/.test(lower)) role_guess = 'color';
          else if (/^--(.*-)?(spacing|space|gap|pad|margin|inset)/.test(lower)) role_guess = 'spacing';
          else if (/^--(.*-)?(font|family|type)/.test(lower)) role_guess = 'font';
          else if (/^--(.*-)?(radius|round|corner)/.test(lower)) role_guess = 'radius';
          else if (/^--(.*-)?(shadow|elevation|depth)/.test(lower)) role_guess = 'shadow';
          else if (/^--(.*-)?(duration|ease|timing|delay|motion)/.test(lower)) role_guess = 'motion';
          else if (/^--(.*-)?(weight|size|leading|line-height|tracking)/.test(lower)) role_guess = 'type-scale';
          out.push({ name, value, role_guess, source: 'extracted' });
          if (out.length >= 200) break; // cap — some sites declare thousands
        }
      } catch (e) { /* permission errors swallowed — partial is fine */ }
      return out;
    })();

    // Reverse-lookup: hex → custom-property name. Used below to refine
    // already-extracted color *names* without an LLM call. For each
    // extracted color, find a custom property whose value resolves to
    // the same hex and lift its name (stripped of leading `--` and
    // common prefixes) into the color's `name` field.
    function findCustomPropertyName(hex) {
      if (!hex) return null;
      const target = hex.toUpperCase();
      for (const prop of css_custom_properties) {
        if (prop.role_guess && prop.role_guess !== 'color') continue;
        const c = parseColor(prop.value);
        if (c && c.hex.toUpperCase() === target) {
          // Strip --brand- / --color- / --colors- prefixes and
          // convert kebab-to-Title-Case so "--brand-color-magenta-500"
          // becomes "Magenta 500".
          return prop.name
            .replace(/^--/, '')
            .replace(/^(brand|color|colour|colors|theme|palette|c|t)-/i, '')
            .replace(/^(text|bg|background|fg|accent|primary|secondary)-/i, '')
            .replace(/-/g, ' ')
    ...
    // Lift CSS-var names onto the canonical color fields when their
    // current name is a generic placeholder. This is the deterministic
    // shortcut that means many publishers won't need LLM color naming.
    const REFINABLE_COLOR_NAMES = new Set([
      'Primary Accent', 'Secondary Accent', 'Theme Color',
      'Primary Text', 'Secondary Text', 'Tertiary Text',
      'Near Black', 'Dark Gray', 'Medium Gray',
      'White', 'Section', 'Secondary', 'Dark Background', 'Off-White',
      'Warning Yellow', 'Negative Red', 'Positive Green', 'Info Blue',
      'Default Blue',
    ]);
    function maybeRefineColorName(entry) {
      if (!entry || !entry.hex) return;
      if (!REFINABLE_COLOR_NAMES.has(entry.name)) return;
      const refined = findCustomPropertyName(entry.hex);
      if (refined && refined.length > 0 && refined.length < 60) {
        entry.name = refined;
        entry.source = entry.source === 'extracted' ? 'extracted' : 'refined';
      }
    }
    maybeRefineColorName(colors.primary);
    if (colors.secondary) maybeRefineColorName(colors.secondary);
    ['primary', 'secondary', 'tertiary'].forEach((k) => maybeRefineColorName(colors.text[k]));
    ['base', 'section', 'secondary', 'dark'].forEach((k) => maybeRefineColorName(colors.backgrounds[k]));
    Object.keys(colors.accents).forEach((k) => maybeRefineColorName(colors.accents[k]));

    // ---- DEEPER TEXT COLORS (6 levels) ----
    // Existing fields (text.primary / secondary / tertiary) stay
    // populated by the original quartile-based picker. Add 3 more
    // sampled directly from the elements where they're used most.
    function sampleSingleColor(selector, prop = 'color') {
      const el = document.querySelector(selector);
      if (!el) return null;
      const c = parseColor(getCS(el, prop));
      if (!c || !isGray(c)) return null;
      return c;
    }
    const deepDarkText = sampleSingleColor('h1') || sampleSingleColor('h2');
    const bodyTextColor = sampleSingleColor('article p, [class*="article-body"] p, [class*="story-body"] p, main p');
    const captionTextColor = sampleSingleColor('figcaption, .caption, [class*="caption"]');
    if (deepDarkText && (!colors.text.primary || deepDarkText.hex !== colors.text.primary.hex)) {
      colors.text.deep_dark = { name: 'Deep Dark', hex: deepDarkText.hex, rgb: deepDarkText.rgb, usage: ['Hero headlines'], source: 'extracted' };
      maybeRefineColorName(colors.text.deep_dark);
    }
    if (bodyTextColor) {
      colors.text.body = { name: 'Body Text', hex: bodyTextColor.hex, rgb: bodyTextColor.rgb, usage: ['Article body'], source: 'extracted' };
      maybeRefineColorName(colors.text.body);
    }
    if (captionTextColor) {
      colors.text.caption = { name: 'Caption', hex: captionTextColor.hex, rgb: captionTextColor.rgb, usage: ['Image captions'], source: 'extracted' };
      maybeRefineColorName(colors.text.caption);
    }

    // ---- UI OVERLAY rgba COLORS ----
    // Translucent backgrounds on badges/overlays/modals get filtered
    // out by parseColor() above (alpha 0 returns null). Capture them
    // explicitly here — they're what give CBS its `rgba(16,16,16,0.65)`
    // video scrim, t-online its modal dim layer, etc.
    function parseRgba(str) {
      if (!str) return null;
      const m = str.match(/rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)(?:\s*,\s*([\d.]+))?\s*\)/);
      if (!m) return null;
      const r = +m[1], g = +m[2], b = +m[3], a = m[4] !== undefined ? parseFloat(m[4]) : 1;
      if (a >= 0.98) return null; // opaque — already captured above
      if (a <= 0.02) return null; // effectively transparent
      const hex = rgbToHex(r, g, b);
      return { hex, rgb: `rgba(${r}, ${g}, ${b}, ${a})`, alpha: a };
    }
    const overlay_sources = {
      video_scrim: '[class*="video"] [class*="overlay"], [class*="play"] [class*="overlay"], [class*="VideoOverlay"], video + [class*="overlay"]',
      modal: '[class*="modal"], [role="dialog"], [class*="Modal"], [class*="lightbox"]',
      sponsored: '[class*="sponsored"], [class*="Sponsored"], [class*="ad-overlay"]',
      tooltip: '[role="tooltip"], [class*="tooltip"], [class*="Tooltip"]',
    };
    const ui_overlays = {};
    for (const [key, sel] of Object.entries(overlay_sources)) {
      const el = document.querySelector(sel);
      if (!el) continue;
      const bg = parseRgba(getCS(el, 'background-color')) || parseRgba(getCS(el, 'background'));
      if (bg) ui_overlays[key] = { ...bg, source: 'extracted' };
    }
    if (Object.keys(ui_overlays).length > 0) colors.ui_overlays = ui_overlays;

    // ---- BORDER / DIVIDER COLORS ----
    // Real publishers distinguish their content dividers from card
    // borders from hr rules — capture each.
    function sampleBorderColor(sel) {
      const el = document.querySelector(sel);
      if (!el) return null;
      const cs = window.getComputedStyle(el);
      const top = parseColor(cs.borderTopColor);
      const bottom = parseColor(cs.borderBottomColor);
      const pick = (top || bottom);
      if (!pick) return null;
      // Skip near-transparent / near-page-bg borders.
      if (luminance(pick) > 250 || luminance(pick) < 5) return null;
      return pick;
    }
    const borders = {};
    const dividerColor = sampleBorderColor('hr') || sampleBorderColor('[class*="divider"]') || sampleBorderColor('[class*="separator"]');
    if (dividerColor) borders.divider = { name: 'Divider', hex: dividerColor.hex, rgb: dividerColor.rgb, source: 'extracted' };
    const cardBorderColor = sampleBorderColor('[class*="card"]') || sampleBorderColor('article');
    if (cardBorderColor) borders.card = { name: 'Card Border', hex: cardBorderColor.hex, rgb: cardBorderColor.rgb, source: 'extracted' };
    if (Object.keys(borders).length > 0) colors.borders = borders;

    // ---- SHOW / SECTION BRAND COLORS ----
    // CBS gives each show (Mornings, 60 Minutes, …) its own colored
    // section band. Detect them by iterating top-level sections whose
    // background differs from the page background.
    const pageBgColor = colors.backgrounds.base?.hex || '#FFFFFF';
    const show_brand_colors = {};
    const sectionEls = document.querySelectorAll(
      'section, article > section, [class*="show-"], [class*="Show-"], section[data-show], section[id]'
    );
    let showCount = 0;
    for (const sec of sectionEls) {
      ...
      const bg = parseColor(cs.backgroundColor);
      if (!bg || bg.hex.toUpperCase() === pageBgColor.toUpperCase()) continue;
      if (luminance(bg) > 245) continue; // skip near-white
      // Skip if the section has no visible content (zero height).
      if (!sec.offsetHeight || sec.offsetHeight < 40) continue;
      // Anchor a label: nearest h2/h3 inside, or data-show / aria-label / id.
      const heading = sec.querySelector('h1, h2, h3');
      const label = (heading && heading.textContent.trim().substring(0, 40))
        || sec.getAttribute('data-show')
        || sec.getAttribute('aria-label')
        || sec.id
        || `Section ${showCount + 1}`;
      const slug = label.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || `section-${showCount + 1}`;
      if (!show_brand_colors[slug]) {
        show_brand_colors[slug] = {
          label,
          hex: bg.hex,
          rgb: bg.rgb,
          source: 'extracted',
    ...
    if (Object.keys(show_brand_colors).length > 0) colors.show_brand_colors = show_brand_colors;

    // ---- COLORS.css_custom_properties (publish the scrape) ----
    if (css_custom_properties.length > 0) {
      colors.css_custom_properties = css_custom_properties;
    }

    // ---- TYPE SCALE EXTENDED (16-role hierarchy) ----
    // The existing `fonts.type_scale` keeps its 10 roles for backwards
    // compat. The extended version captures the publisher's full
    // typography vocabulary, mirroring CBS / NBC reference kits.
    const extendedRoleSelectors = [
      { role: 'hero_headline',       sel: 'h1, [class*="hero"] h1, [class*="lead-story"] h1, [class*="HeroHeadline"]' },
      { role: 'secondary_headline',  sel: 'article h2, [class*="story"] h2, [class*="secondary"] h2' },
      { role: 'right_rail_headline', sel: '[class*="sidebar"] h3, [class*="rail"] h3, aside h3' },
      { role: 'section_heading',     sel: 'h2, .section-title, [class*="section-head"], [class*="SectionTitle"]' },
      { role: 'article_title_card',  sel: '.card h3, [class*="card"] h3, [class*="teaser"] h3' },
      { role: 'article_deck',        sel: '.deck, .standfirst, [class*="deck"], [class*="subtitle"], [class*="dek"]' },
      { role: 'article_body',        sel: 'article p, [class*="article-body"] p, [class*="story-body"] p' },
      { role: 'byline',              sel: '.byline, [class*="byline"], [rel="author"]' },
      { role: 'timestamps',          sel: 'time[datetime], [class*="timestamp"], [class*="published-date"]' },
      { role: 'category_eyebrow',    sel: '[class*="eyebrow"], [class*="kicker"], [class*="pre-label"], [class*="category-label"]' },
      { role: 'sub_navigation',      sel: '[class*="subnav"] a, [class*="sub-nav"] a, nav nav a' },
      { role: 'navigation',          sel: 'nav a, [role="navigation"] a' },
      { role: 'more_link',           sel: 'a[class*="more"], a[class*="MoreLink"], [class*="see-all"]' },
      { role: 'footer_link',         sel: 'footer a' },
      { role: 'buttons',             sel: 'button, .btn, [role="button"]' },
      { role: 'caption',             sel: 'figcaption, .caption' },
    ];
    const type_scale_extended = [];
    for (const { role, sel } of extendedRoleSelectors) {
      const el = document.querySelector(sel);
      if (!el) continue;
      const cs = window.getComputedStyle(el);
      const entry = {
        ...
        selector: sel,
        family: cleanFont(cs.fontFamily.split(',')[0]),
        weight: parseInt(cs.fontWeight) || null,
        size: cs.fontSize,
        line_height: cs.lineHeight === 'normal' ? null : cs.lineHeight,
        letter_spacing: cs.letterSpacing === 'normal' ? null : cs.letterSpacing,
        text_transform: cs.textTransform === 'none' ? null : cs.textTransform,
        color: parseColor(cs.color)?.hex || null,
        font_style: cs.fontStyle === 'normal' ? null : cs.fontStyle,
        source: 'extracted',
      };
      // Strip null keys so the JSON stays tight.
      Object.keys(entry).forEach((k) => entry[k] == null && delete entry[k]);
      type_scale_extended.push(entry);
    }
    if (type_scale_extended.length > 0) {
      fonts.type_scale_extended = type_scale_extended;
    }

    // ---- BUTTON VARIANTS (primary / secondary / outline / soft) ----
    // Existing `buttons.primary` was a single best-effort pick. Layer
    // multiple variants on top by scoring per pattern.
    function scoreButton(el) {
      const cs = window.getComputedStyle(el);
      const bg = parseColor(cs.backgroundColor);
      const border = parseColor(cs.borderTopColor);
      const hasBg = bg && luminance(bg) < 240;
      const hasBorder = border && (border.r + border.g + border.b) < 720;
      return {
        el, cs, bg, border, hasBg, hasBorder,
        height: el.offsetHeight,
        width: el.offsetWidth,
      };
    }
    function buttonSpec(s) {
      return {
        background_color: s.bg?.hex || null,
        text_color: parseColor(s.cs.color)?.hex || null,
        border_color: s.border?.hex || null,
        border_width: s.cs.borderTopWidth,
        border_radius: s.cs.borderRadius,
        padding: s.cs.padding,
        font_size: s.cs.fontSize,
        font_weight: parseInt(s.cs.fontWeight) || 700,
        text_transform: s.cs.textTransform === 'none' ? null : s.cs.textTransform,
        letter_spacing: s.cs.letterSpacing === 'normal' ? null : s.cs.letterSpacing,
        source: 'extracted',
      };
    }
    // Build a deduped registry of button shapes — we don't want to
    // pick three buttons that all look the same.
    const buttonShapes = [];
    const buttonCandidatesAll = document.querySelectorAll('button, [role="button"], .btn, a[class*="btn"], a[class*="Btn"], a[class*="cta"], a[class*="CTA"]');
    for (const el of buttonCandidatesAll) {
      if (el.offsetHeight < 24 || el.offsetWidth < 50) continue;
      const s = scoreButton(el);
      // Dedupe by bg + border + radius signature.
      const sig = `${s.bg?.hex || '_'}-${s.border?.hex || '_'}-${s.cs.borderRadius}`;
      if (buttonShapes.find((b) => b.sig === sig)) continue;
      buttonShapes.push({ sig, ...s });
      if (buttonShapes.length >= 8) break;
    }
    // Classify: solid bg → primary; outline (transparent bg + visible border) → outline;
    // soft bg (light, non-primary) → soft_cta; remainder → secondary.
    function pickByPredicate(pred) {
      return buttonShapes.find(pred) || null;
    }
    const primaryPick = pickByPredicate((s) => s.hasBg && s.bg.hex.toUpperCase() === colors.primary?.hex?.toUpperCase());
    const fallbackPrimary = pickByPredicate((s) => s.hasBg && luminance(s.bg) < 200);
    const outlinePick = pickByPredicate((s) => !s.hasBg && s.hasBorder);
    const softPick = pickByPredicate((s) => s.hasBg && luminance(s.bg) > 200 && luminance(s.bg) < 250);
    const secondaryPick = pickByPredicate(
      (s) => s.hasBg && !s.hasBorder && s !== primaryPick && s !== fallbackPrimary && s !== softPick
    );
    if (outlinePick) buttons.outline = buttonSpec(outlinePick);
    if (softPick) buttons.soft_cta = buttonSpec(softPick);
    if (secondaryPick) buttons.secondary = buttonSpec(secondaryPick);

    // ---- SHADOW REGISTRY ----
    const shadows = (() => {
      const seen = new Set();
      const out = {};
      const probe = (key, selector) => {
        const el = document.querySelector(selector);
        if (!el) return;
        const sh = window.getComputedStyle(el).boxShadow;
        if (!sh || sh === 'none') return;
        if (seen.has(sh)) return;
        seen.add(sh);
        out[key] = { value: sh, selector, source: 'extracted' };
      };
      probe('card', '[class*="card"], [class*="Card"], article');
      probe('button', 'button, .btn, [role="button"]');
      probe('popup', '[class*="popup"], [class*="dropdown"], [role="dialog"], [class*="modal"]');
      probe('elevated', '[class*="elevated"], [class*="hero"], [class*="featured"]');
      return out;
    })();
    ...
    const border_radius = (() => {
      ...
      const probe = (key, selector) => {
        ...
        if (!r || r === '0px' && key !== 'images') return;
        out[key] = r;
      };
      probe('buttons', 'button, .btn');
      probe('cards', '[class*="card"], article');
      probe('images', 'article img, .card img');
      probe('overlays', '[class*="modal"], [role="dialog"]');
      probe('pills', '[class*="pill"], [class*="chip"], [class*="badge"]');
      probe('inputs', 'input[type="text"], input[type="search"], input[type="email"]');
      return out;
    })();

    // ---- ICON CATALOG ----
    // Iterate every SVG with an aria-label / title / role hint and
    // capture its viewBox + nearest interactive label. Bounded at 60
    // entries to keep payload sane.
    const icon_catalog = [];
    const svgEls = document.querySelectorAll('svg');
    const seenIcons = new Set();
    for (let i = 0; i < svgEls.length && icon_catalog.length < 60; i++) {
      const svg = svgEls[i];
      const ariaLabel = svg.getAttribute('aria-label')
        || svg.querySelector('title')?.textContent?.trim()
        || null;
      const role = svg.getAttribute('role') || null;
      const viewBox = svg.getAttribute('viewBox') || null;
      // Heuristic role hint: nearest button/link's aria-label or visible text.
      const parentInteractive = svg.closest('button, a, [role="button"], [role="link"]');
      const roleHint = parentInteractive
        ? (parentInteractive.getAttribute('aria-label')
            || parentInteractive.textContent.trim().substring(0, 40)
            || null)
        : null;
      // Dedupe by aria-label OR role hint OR viewBox + first-path-d signature.
      const firstPath = svg.querySelector('path');
      const sig = ariaLabel || roleHint || (viewBox && firstPath && (viewBox + ':' + (firstPath.getAttribute('d') || '').substring(0, 40))) || null;
      if (!sig || seenIcons.has(sig)) continue;
      seenIcons.add(sig);
      icon_catalog.push({
        ...
        view_box: viewBox,
        role_hint: roleHint,
        width: svg.getBoundingClientRect().width || null,
        height: svg.getBoundingClientRect().height || null,
        source: 'extracted',
      });
    }
    if (icon_catalog.length > 0) icons.catalog = icon_catalog;

    // ---- HEADER LAYER BREAKDOWN ----
    // Replace the existing one-line "tag: text" strings with typed
    // per-layer objects that capture bg/text/height — exactly what
    // the loader and prototype need to render each layer with the
    // right surface treatment.
    if (headerEl) {
      const layers_detailed = [];
      const children = Array.from(headerEl.children).slice(0, 5);
      for (const child of children) {
        ...
        const text = child.textContent.trim().substring(0, 50);
        const tag = child.tagName.toLowerCase();
        layers_detailed.push({
          tag,
          label: text || '(visual element)',
          height: child.offsetHeight,
          background_color: bg?.hex || null,
          text_color: txt?.hex || null,
          is_dark: bg ? luminance(bg) < 128 : null,
          source: 'extracted',
        });
      }
      if (layers_detailed.length > 0) {
        layout_patterns.header.layers_detailed = layers_detailed;
      }
    }

    // ---- SPACING SCALE (from CSS custom properties) ----
    const spacing_scale = (() => {
      const out = {};
      const tiers = [
        { key: 'base',   regex: /^--(.*-)?(spacing|space|gap)-(0|base|md|default|4|3)$/i },
        { key: 'small',  regex: /^--(.*-)?(spacing|space|gap)-(sm|small|2|3)$/i },
        { key: 'medium', regex: /^--(.*-)?(spacing|space|gap)-(md|medium|4|5)$/i },
        { key: 'large',  regex: /^--(.*-)?(spacing|space|gap)-(lg|large|6|7|8)$/i },
        { key: 'xl',     regex: /^--(.*-)?(spacing|space|gap)-(xl|9|10|12)$/i },
        { key: 'xxl',    regex: /^--(.*-)?(spacing|space|gap)-(xxl|2xl|3xl|14|16|20)$/i },
      ];
      for (const tier of tiers) {
        const match = css_custom_properties.find((p) => tier.regex.test(p.name));
        if (match) out[tier.key] = match.value;
      }
      return out;
    })();
    if (Object.keys(spacing_scale).length > 0) {
      layout_patterns.spacing_scale = spacing_scale;
    }
    ...
    const max_widths = (() => {
      const out = {};
      const targets = [
        { key: 'site_container', sel: '.container, [class*="Container"], [class*="page-container"], main' },
        { key: 'card_wide',      sel: '[class*="card-wide"], [class*="card-large"], article' },
        { key: 'card_narrow',    sel: '[class*="card-narrow"], [class*="card-small"], [class*="rail"]' },
        { key: 'article_column', sel: '[class*="article-body"], [class*="story-body"], article > div' },
      ];
      for (const { key, sel } of targets) {
        const el = document.querySelector(sel);
        if (!el) continue;
        const mw = window.getComputedStyle(el).maxWidth;
        if (mw && mw !== 'none' && parseInt(mw) >= 400 && parseInt(mw) <= 1800) {
          out[key] = mw;
        }
      }
      return out;
    })();
    if (Object.keys(max_widths).length > 0) {
      layout_patterns.max_widths = max_widths;
    }
    ...
    const extractionQuality = (() => {
      // Five-bucket provenance — see Phase 1 plan. `refined` is a Layer-1
      // win where the crawler renamed an extracted color using its CSS
      // custom-property name (still authoritative; counted as extracted).
      // `derived` (color variants) and `enriched` (future LLM step) are
      // tracked separately so the analysis report can surface confidence.
      const tally = {
        extracted: 0, refined: 0, derived: 0, enriched: 0,
        fallback: 0, fallbackTokens: [],
      };
      const visit = (path, value) => {
        ...
        if (typeof value.source === 'string') {
          if (value.source === 'fallback') {
            ...
            tally.extracted++;
          } else if (value.source === 'refined') {
            tally.refined++;
          } else if (value.source === 'derived') {
            tally.derived++;
          } else if (value.source === 'enriched') {
            tally.enriched++;
          }
        }
      ...
      visit('colors', colors);
      visit('fonts', fonts);
      visit('buttons', buttons);
      // Treat refined as authoritative (the page told us this name).
      const authoritative = tally.extracted + tally.refined;
      const total = authoritative + tally.fallback + tally.derived + tally.enriched;
      const ratio = total === 0 ? 0 : authoritative / total;
      return {
    ...
    return {
      brand, logos, colors, fonts, brand_voice, photo_style, graphics, icons,
      layout_patterns, spacing, buttons,
      shadows: Object.keys(shadows).length > 0 ? shadows : undefined,
      border_radius: Object.keys(border_radius).length > 0 ? border_radius : undefined,
      extraction_quality: extractionQuality,
    };
  });

  // Add metadata. crawler_version + analysis_method are explicit so the
  // analysis report can surface what this run produced — Phase 1 added
  // CSS-var color naming, shadow registry, border-radius map, spacing
  // scale, 16-role type hierarchy, button variants, deeper text colors,
  // rgba UI overlays, dividers, show-brand colors, icon catalog, header
  // layer breakdown.
  extracted.metadata = {
    analysis_date: new Date().toISOString().split('T')[0],
    source_url: url,
    crawler_version: '2.2.0',
    analysis_method: 'Puppeteer computed-style extraction (deep) — Phase 1',
