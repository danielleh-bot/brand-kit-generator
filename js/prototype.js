...
function generatePrototype() {
    ...
    const heroHtml = heroSrc
        ? `<img src="${_escAttr(heroSrc)}" style="width:100%;aspect-ratio:16/9;object-fit:cover;display:block;" crossorigin="anonymous" onerror="this.style.display='none'" />`
        : '';

    // Image helper for feed cards — shows real image or colored placeholder with initials
    const cardImage = (card, size) => {
        const src = card.thumbnail || card.image || '';
        if (src) {
            return `<img src="${_escAttr(src)}" alt="" loading="lazy" style="width:100%;aspect-ratio:4/3;object-fit:cover;display:block;border-radius:${borderRadius} ${borderRadius} 0 0;background:${bgSection};" crossorigin="anonymous" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" /><div style="display:none;width:100%;aspect-ratio:4/3;background:${bgSection};align-items:center;justify-content:center;color:${textTertiary};font-size:14px;border-radius:${borderRadius} ${borderRadius} 0 0;">${_escHtml(card.source || card.category || '')}</div>`;
        }
        // No image at all — subtle colored placeholder
        return `<div style="width:100%;aspect-ratio:4/3;background:${bgSection};display:flex;align-items:center;justify-content:center;color:${textTertiary};font-size:14px;border-radius:${borderRadius} ${borderRadius} 0 0;">${_escHtml(card.source || card.category || '')}</div>`;
    };
    ...
    const sectionLabel = (text) => `
        <div style="font-family:'${fontPrimary}',sans-serif;font-size:${sectionScale.size};font-weight:${sectionScale.weight};text-transform:uppercase;letter-spacing:0.05em;color:${textSecondary};padding-bottom:8px;margin-bottom:16px;border-bottom:3px solid ${primary};text-align:${startSide};">
            <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${primary};${marginStart}:8px;vertical-align:middle;"></span>
            ${_escHtml(text)}
        </div>
    `;
    ...
    const listCard = (card) => {
        const src = card.thumbnail || card.image || '';
        const imgHtml = src
            ? `<img src="${_escAttr(src)}" alt="" loading="lazy" style="width:120px;height:80px;object-fit:cover;border-radius:${borderRadius};flex-shrink:0;background:${bgSection};" crossorigin="anonymous" onerror="this.style.display='none'" />`
            : `<div style="width:120px;height:80px;background:${bgSection};border-radius:${borderRadius};flex-shrink:0;display:flex;align-items:center;justify-content:center;color:${textTertiary};font-size:11px;">${_escHtml(card.source || '')}</div>`;
...
<body>
    ...
    <nav style="background:${bgDark};color:#fff;padding:0;">
        <div style="background:rgba(0,0,0,0.2);padding:4px 0;font-size:12px;">
            <div style="max-width:1200px;margin:0 auto;padding:0 16px;display:flex;justify-content:space-between;align-items:center;">
                <span style="opacity:0.7;">${_escHtml(new Date().toLocaleDateString(lang, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }))}</span>
                <span style="opacity:0.7;">${_escHtml(domain)}</span>
            </div>
        </div>
        <div style="max-width:1200px;margin:0 auto;padding:12px 16px;display:flex;align-items:center;gap:32px;${isRTL ? 'flex-direction:row-reverse;' : ''}">
            <a href="#" style="font-family:var(--font-headline);font-size:24px;font-weight:700;color:#fff;text-decoration:none;">${_escHtml(pub)}<span style="color:${primary};">.</span></a>
    ...
    <div style="max-width:760px;margin:0 auto;padding:0 16px 40px;">
        <div style="border-top:3px solid ${primary};padding-top:16px;margin-bottom:24px;">
            ${sectionLabel(feedContent.sponsoredLabel + ' Stories')}
        </div>

        <!-- Large Sponsored (2-up grid) -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:32px;">
            ${feedContent.sponsoredLarge.map(c => feedCard(c, 'large')).join('')}
        </div>

        <!-- Dense Sponsored (list format) -->
        <div style="margin-bottom:32px;">
            ${feedContent.sponsoredDense.map(c => listCard(c)).join('')}
        </div>

        <!-- Native Section: More From Publisher -->
        <div style="margin-bottom:32px;">
            ${sectionLabel(feedContent.moreFromLabel + ' ' + pub)}
        ...
        <div style="margin-bottom:32px;">
            ${sectionLabel(feedContent.trendingLabel + ' ' + pub)}
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:14px;">
                ${feedContent.trendingNative.map(c => feedCard(c, 'tiny')).join('')}
            </div>
        </div>

        <!-- Final Sponsored (list format) -->
        <div style="margin-bottom:20px;">
            ${feedContent.sponsoredFinal.map(c => listCard(c)).join('')}
        </div>

        <!-- Attribution -->
        <div style="text-align:center;padding:16px 0;font-size:11px;color:${textTertiary};">
            by <strong>Taboola</strong> | ${_escHtml(feedContent.sponsoredLabel)} Links
        </div>
    </div>
    ...
    <footer style="background:${bgDark};color:rgba(255,255,255,0.7);padding:48px 16px 24px;">
        <div style="max-width:1200px;margin:0 auto;">
            ...
            ${nav.socialLinks.length > 0 ? `<div style="display:flex;gap:16px;margin-bottom:24px;">${nav.socialLinks.map(s => `<a href="#" style="color:rgba(255,255,255,0.5);font-size:13px;text-decoration:none;">${_escHtml(s.platform)}</a>`).join('')}</div>` : ''}
